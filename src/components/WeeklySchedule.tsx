...
const WeeklySchedule = ({
  ...
  return (
    <div
      ref={containerRef}
      className="min-w-full relative grid grid-cols-[repeat(7,minmax(120px,1fr))] border border-b-0 border-border"
    >
      {/* Table Header */}
      ...
      {hours.map((hour) => (
        <React.Fragment key={hour}>
          ...
          {WEEKDAYS.map((day, index) => (
            <div
              key={day + hour}
              className={`relative border-r ${index === WEEKDAYS.length - 1 && "border-r-0"} border-b border-border ${
